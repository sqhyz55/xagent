...
class TestSchemaMigrationCompatibility:
    ...
    def sample_documents(self, tmp_path: Path) -> Dict[str, Path]:
        ...
        docs["json"].write_text(json.dumps({
...
schema evolution migration compatibility
    ...
    def test_ingestion_with_new_schema_fields(
        ...
        optional_new_fields = [
            ...
            "metadata", "chunk_count", "status"
        ]
        # At least some new fields should be present
        new_fields_present = any(f in result for f in optional_new_fields)
        ...
        for doc_key, file_path in sample_documents.items():
            with open(file_path, "rb") as f:
                response = client.post(
                    "/api/kb/upload/",
                    files={
                        "file": (file_path.name, f, "text/plain" if doc_key == "text" else
                                "application/json" if doc_key == "json" else "text/markdown")
                    },
    ...
    def test_field_absence_graceful_degradation(
        ...
        """
        # Create a collection
        file_path = Path(__file__).parent / "test_data.txt"
    ...
    def test_mixed_schema_crud_operations(
        ...
        try:
            ...
            with open(temp_path, "rb") as f:
                ...
                assert response.status_code in [200, 409]  # 409 if duplicate not allowed

            # DELETE: Delete document
            if file_id:
                response = client.delete(
                    f"/api/kb/documents/?file_id={file_id}",
                    headers=auth_headers
                )
...
class TestBehaviorConsistency:
    ...
    def test_search_consistency_before_after_migration(
        ...
        test_docs = [
            ("doc1.txt", "Python is a programming language for data science and machine learning"),
            ("doc2.txt", "JavaScript is used for web development and frontend applications"),
            ("doc3.txt", "Python and JavaScript are both popular programming languages")
        ]
        ...
        for result in results:
            ...
            assert any(k in result for k in ["metadata", "score", "distance", "relevance"])
    ...
    def test_frontend_display_consistency(
        ...
        try:
            ...
            for collection in collections:
                ...
                assert any(k in collection for k in ["document_count", "doc_count", "count", "size"])
            ...
            for doc in documents:
                ...
                assert "metadata" in doc or any(k.startswith("meta") for k in doc.keys())
