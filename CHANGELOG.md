...
- **Chunk multi-page metadata and document page stats (breaking)**
  Chunking now preserves multi-page origins via `metadata["positions"]` (a list of page numbers) and derives `page_number` from the first page when missing. Parse records also persist document-level page statistics (`page_count`, `page_numbers`) in `params_json`.
